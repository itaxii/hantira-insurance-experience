...
1. Open `/control`.
2. Create a room or use the displayed room.
3. Open `/present` on the projector.
4. Start the experience.
5. Show the one-time QR scene.
6. Wait for nicknames and check participant count.
7. Press `ابدأ الرحلة` from control when ready.
8. Use Next/Previous or keyboard controls for beat timing.
9. On interaction scenes, open voting only when ready.
10. Close voting before discussing answers.
