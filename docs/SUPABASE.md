...
3. Enable Realtime for `rooms`, `participants`, and `votes` if it is not already enabled by the migration.
...
`participants` stores `room_id`, `session_id`, display name, and timestamps. `display_name` is not unique.
...
Rooms are readable by anonymous and authenticated clients so audience screens can follow state. Only authenticated presenters can create/update rooms they own. Anonymous participants can join open rooms and vote only while the current room has voting open for the active interaction.
