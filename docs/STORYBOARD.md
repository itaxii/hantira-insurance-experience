...
| 3 | `hantira-opinion` | interaction | ask → reaction | — | Interaction 1 — "حنتيرة عنده حق؟" (single, allowChange). Beat 2 is a reaction beat, not a duplicate question. |
| 4 | `nice-morning` | story | time → drive → crash → invoice | `sunny-road`, `driving`, `crash`, `invoice` | Monday 8:30 AM. Parallax drive → BAAAAM (screen shake + sound cue, detached wheel, smoke) → invoice count-up 0 → 350,000 EGP with “يا نهار أبيض مين اللي هيدفع الفلوس دي؟”. |
| 5 | `who-pays` | interaction | ask | `invoice` | Interaction 2 — correct answer D (coverage-dependent). The beat headline anchors on 350,000 EGP while the interaction panel renders the single question. |
...
| 11 | `complexity` | story | terms → freeze | `term-cloud`, `term-cloud-tighten` | Typographic chaos (Premium, Deductible, Limits, Exclusions, Conditions, Claims, Add-ons, Coverage) then a tightened frozen frame. |
...
| 15 | `build-protection` | interaction | setup → vote → map → payoff | `logistics`, `protection-vote`, `protection-map`, `shield` | Interaction 4 — multi-select (Motor, Property, Medical, Marine Cargo, Liability, Fire). The phone question is active only on `vote` and `map`; payoff returns to story mode with the shield. |
...
| 24 | `contact-reveal` | story | reveal → who-we-are → who-we-serve → what-we-do | `contact-reveal`, `contact-stats`, `contact-serves`, `contact-flow` | Contact-branded reveal using the supplied logo and public company facts from the provided deck/site: 11+ years, 40K+ clients annually, #5 ranking, and the service flow. |
